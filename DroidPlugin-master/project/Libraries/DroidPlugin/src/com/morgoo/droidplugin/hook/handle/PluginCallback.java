        ...
        try {
            ...
            if (PluginProcessManager.isPluginProcess(mHostContext)) {
                if (!PluginManager.getInstance().isConnected()) {
                    //ËøôÈáåÂøÖÈ°ªË¶ÅËøô‰πàÂÅö„ÄÇÂ¶ÇÊûúÂΩìÂâçËøõÁ®ãÊòØÊèí‰ª∂ËøõÁ®ãÔºåÂπ∂‰∏îÔºåËøòÊ≤°ÊúâÁªëÂÆö‰∏äÊèí‰ª∂ÁÆ°ÁêÜÊúçÂä°ÔºåÊàë‰ª¨ÂàôÊääÊ∂àÊÅØÂª∂Ëøü‰∏ÄÊÆµÊó∂Èó¥ÂÜçÂ§ÑÁêÜ„ÄÇ
                    //ËøôÊ†∑ËôΩÁÑ∂‰ºöÈôç‰ΩéÂêØÂä®ÈÄüÂ∫¶Ôºå‰ΩÜÊòØÂèØ‰ª•Ëß£ÂÜ≥Âú®Ê≤°ÁªëÂÆöÊúçÂä°Â∞±ÂêØÂä®Ôºå‰ºöÂØºËá¥ÁöÑ‰∏ÄÁ≥ªÂàóÊó∂Â∫èÈóÆÈ¢ò„ÄÇ
                    Log.i(TAG, "handleMessage not isConnected post and wait,msg=%s", msg);
                    mOldHandle.sendMessageDelayed(Message.obtain(msg), 5);
                    //ËøîÂõûtrueÔºåÂëäËØâ‰∏ãÈù¢ÁöÑhandle‰∏çË¶ÅÂ§ÑÁêÜ‰∫Ü„ÄÇ
                    return true;
                }
            }
            ...
            if (msg.what == LAUNCH_ACTIVITY) {
                ...
                //TODO Â§ÑÁêÜCREATE_BACKUP_AGENT
            } else if (msg.what == DESTROY_BACKUP_AGENT) {
                //TODO Â§ÑÁêÜDESTROY_BACKUP_AGENT
...
//        //ÂÖ∂ÂÆûËøôÈáå‰ªÄ‰πàÈÉΩ‰∏çÁî®ÂÅöÁöÑ„ÄÇ
//        try {
//            Object obj = msg.obj;
//            Intent intent = (Intent) FieldUtils.readField(obj, "intent", true);
//            intent.setExtrasClassLoader(getClass().getClassLoader());
    ...
    private boolean handleLaunchActivity(Message msg) {
        try {
            ...
            Intent targetIntent = stubIntent.getParcelableExtra(Env.EXTRA_TARGET_INTENT);
         	// ’‚¿Ô∂‡º”“ª∏ˆisNotShortcutProxyActivityµƒ≈–∂œ£¨“ÚŒ™ShortcutProxyActivityµƒ∫‹Ãÿ ‚£¨∆Ù∂ØÀ¸µƒ ±∫Ú£
         	// “≤ª·¥¯…œ“ª∏ˆEXTRA_TARGET_INTENTµƒ ˝æ›£¨æÕª·µº÷¬’‚¿ÔŒÛ“‘Œ™ «∆Ù∂Ø≤Âº˛Activity£¨À˘“‘’‚¿Ô“™œ»◊ˆ“ª∏ˆ≈–∂œ°£
         	// ÷Æ«∞ShortcutProxyActivity¥ÌŒÛ∏¥”√¡Àkey£¨µ´ «Œ™¡ÀºÊ»›£¨À˘“‘’‚¿ÔæÕœ»’‚√¥≈–∂œ∞…°£
