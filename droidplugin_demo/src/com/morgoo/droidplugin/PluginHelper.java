...
public class PluginHelper implements ServiceConnection {
	...
	private void initPlugin(Context baseContext) {
		...
		try {
			...
			Log.i(TAG, "Init plugin in process cost %s ms",
					(System.currentTimeMillis() - b));
		}
	}

	// 解决小米JLB22.0 4.1.1系统自带的小米安全中心（lbe.security.miui）广告拦截组件导致的插件白屏问题
	private void fixMiUiLbeSecurity() throws ClassNotFoundException,
			NoSuchFieldException, NoSuchMethodException,
			InvocationTargetException, IllegalAccessException {

		// 卸载掉LBE安全的ApplicationLoaders.mLoaders钩子
		Class ApplicationLoaders = Class
				.forName("android.app.ApplicationLoaders");
		Object applicationLoaders = MethodUtils.invokeStaticMethod(
				ApplicationLoaders, "getDefault");
		Object mLoaders = FieldUtils.readField(applicationLoaders, "mLoaders",
				true);
		if (mLoaders instanceof HashMap) {
			...
			if ("com.lbe.security.client.ClientContainer$MonitoredLoaderMap"
				...
				FieldUtils.writeField(applicationLoaders, "mLoaders", value,
						true);
			}
		}

		// 卸载掉LBE安全的ActivityThread.mPackages钩子
		Object currentActivityThread = ActivityThreadCompat
				.currentActivityThread();
		Object mPackages = FieldUtils.readField(currentActivityThread,
				"mPackages", true);
		if (mPackages instanceof HashMap) {
			...
			if ("com.lbe.security.client.ClientContainer$MonitoredPackageMap"
				...
				HashMap value = new HashMap();
				value.putAll(oldValue);
				FieldUtils.writeField(currentActivityThread, "mPackages",
						value, true);
			}
		}
		...
		if (Looper.getMainLooper() == Looper.myLooper()) {
			...
			try {
				Object mMessages = FieldUtils.readField(queue, "mMessages",
						true);
				if (mMessages instanceof Message) {
					findLbeMessageAndRemoveIt((Message) mMessages);
				}
				Log.e(TAG, "getMainLooper MessageQueue.IdleHandler:"
						+ mMessages);
			} catch (Exception e) {
	...
	private void findLbeMessageAndRemoveIt(Message message) {
		...
		if (message.what == 0 && callback != null) {
			if (callback.getClass().getName()
					.indexOf("com.lbe.security.client") >= 0) {
		...
		try {
			...
			Log.e(TAG, "findLbeMessageAndRemoveIt:error on remove lbe message",
					e);
		}
